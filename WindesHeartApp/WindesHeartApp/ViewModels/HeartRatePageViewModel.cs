...
namespace WindesHeartApp.ViewModels
{
    public class HeartRatePageViewModel : INotifyPropertyChanged
    {
        ...
        public void PreviousDay2BtnClick(object sender, EventArgs args)
        {
            _dateTime = _dateTime.AddHours(-24);
            _dateTime2 = _dateTime2.AddHours(-24);
            DayLabelText = $"{_dateTime.ToString()} - {_dateTime2.ToString()}";
            DrawChart();
            DrawLabels();
        ...
        public void NextDay2BtnClick(object sender, EventArgs args)
        {
            if (!(_dateTime2.AddHours(1) > DateTime.Now))
            {
                _dateTime = _dateTime.AddHours(24);
        ...
        public void ExportBtnClick(object sender, EventArgs e)
        {
            System.Console.WriteLine("Hi");
            //await DependencyService.Get<ISave>().SaveAndView("export.csv", _heartrates);

            DependencyService.Get<IBackgroundDependency>().ExecuteCommand("export.csv", _heartrates);
            /*
            Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.))
            //string path = Path.Combine(Environment.GetExternalStoragePublicDirectory(Environment.DirectoryDocuments).AbsolutePath, "export.csv");
            var heartrates = _heartrates;
            try {
                using (StreamWriter file = new StreamWriter(@path, false)) {
                    System.Console.WriteLine("Exporting...");
                    file.WriteLine("Heartrate, CreatedAt");
                    foreach (Heartrate h in heartrates)
                    {
                        file.WriteLine(h.HeartrateValue + "," + h.DateTime);
                    }
                }
            }
            catch (Exception ex) {
